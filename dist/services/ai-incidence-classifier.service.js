...
exports.AiClassifierProviderError = exports.AiClassifierConfigurationError = void 0;
exports.aiModel = aiModel;
exports.classifyIncidenceWithAi = classifyIncidenceWithAi;
exports.validateClassification = validateClassification;
class AiClassifierConfigurationError extends Error {
}
exports.AiClassifierConfigurationError = AiClassifierConfigurationError;
class AiClassifierProviderError extends Error {
}
exports.AiClassifierProviderError = AiClassifierProviderError;
...
function durationMs(value) {
    ...
    if (Number.isFinite(plainSeconds))
        return Math.ceil(plainSeconds * 1000);
    let total = 0;
    let matched = false;
...
function retryDelayMs(response, attempt, detail = '') {
    ...
    const bodyRetry = durationMs(detail.match(/(?:try again|retry)\s+in\s+(\d+(?:\.\d+)?\s*(?:ms|s|m|h))/i)?.[1] || null);
    const candidates = [
        retryAfterMs,
...
function responseSchema(activeTypes) {
    return {
        type: 'object',
...
async function classifyIncidenceWithAi(context) {
    ...
    const requestBody = JSON.stringify({
        ...
        input: 'Clasifica esta incidencia usando exclusivamente el catalogo activo incluido en el JSON:\n' +
    ...
    for (let attempt = 1; attempt <= MAX_PROVIDER_ATTEMPTS; attempt++) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new AiClassifierProviderError(`OpenAI respondio ${response.status} tras ${attempt} intento${attempt === 1 ? '' : 's'}: ${detail}`);
            }
            const payload = (await response.json());
            if (payload?.status === 'incomplete') {
                throw new AiClassifierProviderError(`OpenAI no completo la respuesta: ${payload?.incomplete_details?.reason || 'motivo desconocido'}.`);
            }
            const text = payload?.output
                ?.flatMap((item) => item?.content || [])
                ?.find((item) => item?.type === 'output_text')?.text;
            if (!text)
                throw new AiClassifierProviderError('OpenAI no devolvio una clasificacion.');
            return validateClassification(JSON.parse(text), context.availableIncidentTypes);
        }
        catch (error) {
            if (error instanceof AiClassifierConfigurationError || error instanceof AiClassifierProviderError)
                throw error;
            const retriable = error.name === 'AbortError' || /fetch failed/i.test(error.message);
            if (retriable && attempt < MAX_PROVIDER_ATTEMPTS) {
            ...
            throw new AiClassifierProviderError(`No se pudo clasificar con OpenAI: ${error.message}`);
        }
        finally {
...
function validateClassification(value, activeTypes) {
    const allowed = new Set(['APLICA', 'NO_APLICA']);
